...
from latency_map.latency_map import LatencyMapV2
...
from retrowrite.librw.loader import Loader
from retrowrite.librw.rw import Rewriter

# from retrowrite.rwtools.nemesis.graph.control_flow_graph import ControlFlowGraph
# from retrowrite.rwtools.nemesis.graph.nemesis_node import NemesisNode
# from retrowrite.rwtools.nemesis.latency_map.latency_map import LatencyMapV2
...
class NemesisInstrumentFunction:
    def __init__(self, loader, target_function):
        ...
        self.leaves = None
        latency_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "latency_map", "latencies.p")
        # self.latency_mapper = LatencyMapV2(load_latency_map("src/latency_map/latencies.p"))
        self.latency_mapper = LatencyMapV2(load_latency_map(latency_file))
        self._setup()
    ...
    def do_analysis(self):
        ...
        for addr, function in self.loader.container.functions.items():
            # TODO dit kan toch niet enkel main zijn?
            # TODO dit was voordien standaar 'main', check of alles nog werkt
            if function.name == self.target_function:
